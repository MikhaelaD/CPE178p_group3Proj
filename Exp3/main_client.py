...
def main(page: ft.Page):
    ...
    result_text = ft.Text("Predicted Class: ")
    result_text_resnet = ft.Text("Predicted Class: ")
    import tempfile
    ...
    async def send_prediction_request(image_data):
        async with websockets.connect("ws://localhost:8000/ws") as websocket:
            ...
            if data.get("type") == "prediction":
                ...
                result_text_resnet.value = f"Predicted Class: {data.get('class_rn')} ({data.get('output_rn')})"
                selected_image.controls[2].controls[0].content.value = f"Predicted Class (mobilenet): {data.get('class')} ({data.get('output')})"
                selected_image.controls[2].controls[1].content.value = f"Predicted Class (resnet): {data.get('class_rn')} ({data.get('output_rn')})"
            else:
                result_text.value = "Error occurred during prediction"
                result_text_resnet.value = "Error occurred during prediction"
